...
class TilesIndex{
    ...
    for (int i = 0;i<animated_tiles.size();i++){
        ...
        for (int j=0;j<frames_nb;j++){
            texture_morning.loadFromFile("assets/tiles/a_morning.png",sf::IntRect(x*16,(j*16)+(y*16),xsize*16,ysize*16));
            animated_morning_textures[i].push_back(texture_morning);
        };

        for (int j=0;j<frames_nb;j++){
            texture_day.loadFromFile("assets/tiles/a_day.png",sf::IntRect(x*16,(j*16)+(y*16),xsize*16,ysize*16));
            animated_day_textures[i].push_back(texture_day);
        };

        for (int j=0;j<frames_nb;j++){
            texture_night.loadFromFile("assets/tiles/a_night.png",    sf::IntRect(x*16,(j*16)+(y*16),xsize*16,ysize*16));
